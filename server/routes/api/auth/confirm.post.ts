...
export default defineEventHandler(async (event) => {
  try {
    ...
    const record = db().prepare('SELECT * FROM codes WHERE user_id=? AND kind=? AND code=? AND used_at IS NULL ORDER BY created_at DESC').get(user.id, 'signup', body.code || '') as unknown as { id: string, expires_at: number } | undefined
    ...
    updateUser(user.id, { status: 'CONFIRMED', attributes: { email_verified: 'true' } })
    return {}
