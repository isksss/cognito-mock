...
VOLUME ["/data"]
EXPOSE 9999
HEALTHCHECK --interval=10s --timeout=3s --start-period=10s --retries=3 CMD ["node", "-e", "fetch('http://127.0.0.1:9999/health').then(r=>{if(!r.ok)process.exit(1)}).catch(()=>process.exit(1))"]
