...
[`example/`](example/) に、cognito-mock でログインしてユーザーを管理する Nuxt 4 サンプルがあります。
...
- 初期ユーザー: `developer@example.com`
- 初期パスワード: `Password123!`

ID・パスワード認証と Managed Login（Authorization Code + PKCE）の両方を試せます。ユーザーCRUD、パスワード、有効状態、グループ所属の管理例を含みます。詳細とE2Eテスト手順は [`example/README.md`](example/README.md) を参照してください。
...
Node.js 24.18.0 と pnpm 11.11.0 を mise で管理します。ルートの `mise.toml` はサービス本体と `example` の両方に適用されます。
...
- `mise run dev`: サービス（9999）とサンプル（3001）を並列起動
- `mise run compose`: Docker Composeで両方を起動
- `mise run e2e`: 実際のCompose環境に対するPlaywrightテスト
